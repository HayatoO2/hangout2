class PresentsController < ApplicationController
  ...
  def new
    ...
    if params[:present] == nil 
      ...
      @presents = Present.where('enter_time <= ? and leave_time > ?',Date.today,Date.today).where(share_id: params[:share_id])
    else
      @present = Present.new(enter_time: params[:present][:enter_time],  share_id: params[:share_id], user_id: current_user.id)
      @presents = Present.where('enter_time <= ? and leave_time >= ?', @present.enter_time,@present.enter_time).where(share_id: params[:share_id])
